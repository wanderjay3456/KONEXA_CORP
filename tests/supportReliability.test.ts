...
test('every help article has unique IDs and complete native copy in all three languages', () => {
  ...
  for (const entry of SUPPORT_ARTICLES) for (const locale of ['ko', 'en', 'vi'] as const) {
...
test('support input limits and locale are checked before any provider call', () => {
  for (const input of [null, {}, {question: '',locale:'ko'}, {question:'x'.repeat(1001),locale:'en'}, {question:'hi',locale:'unknown'}, {question:'hi',locale:'en',articleId:'fake'}]) assert.throws(() => parseSupportRequest(input));
  assert.equal(parseSupportRequest({question:' hi ',locale:'vi'}).question, 'hi');
});
test('public help API answers without auth or a provider and does not echo malicious input', async () => {
  const app = express(); app.set('trust proxy', 1); app.use(express.json()); registerSupportRoutes(app);
...
test('AI assessments reject missing, fabricated or invalid numeric fields', () => {
  for (const value of [undefined, null, '87', NaN, -1, 101]) assert.throws(() => requireAssessmentScore(value,'score'));
...
test('realtime refresh coalesces bursts and never lets a stale response win', async () => {
  ...
  const work = refresh.refresh(); void refresh.refresh(); void refresh.refresh();
  assert.equal(resolvers.length,1);
  resolvers[0](1); await new Promise(resolve => setImmediate(resolve));
  assert.equal(resolvers.length,2);
  resolvers[1](2); await work;
  assert.deepEqual(results,[1,2]);
  const pending = refresh.refresh(); refresh.stop(); resolvers[2](3); await pending;
  assert.deepEqual(results,[1,2]);
});
test('refresh errors stay errors, and the next refresh can recover', async () => {
  let calls=0; const errors: unknown[]=[]; const results:number[]=[];
  const source=createSnapshotRefresh(async()=>{if(++calls===1)throw new Error('offline');return 2;},value=>results.push(value),error=>errors.push(error));
  await source.refresh(); await source.refresh();
  assert.equal(errors.length,1); assert.deepEqual(results,[2]); source.stop();
});
