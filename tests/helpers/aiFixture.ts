...
export function aiFixture(options: { generate?: (request: Row) => Promise<any> } = {}) {
  ...
  const generate = async (request: Row) => generateWithModelFallback(['first', 'second'], async model => {
    ...
    let value: unknown;
    if (input.profileEvidence) value = { strengthSummary: 'Declared research skills support an initial project discussion.', weaknessSummary: 'Verify the work sample and weekly availability.', verificationQuestions: ['What was your contribution to the research work?', 'How many hours can you commit each week?', 'What language can you use for a project discussion?'], evidenceRequests: ['A research brief with confidential details removed.'], skillGap: ['Research evidence'], recommendedSkills: ['Interviews'], recommendedProjects: ['Research brief'], recommendedCompanies: ['Research teams'], recommendedLearningPath: ['Practice structured research interviews.'], careerReadiness: 50, employabilityScore: 50 };
