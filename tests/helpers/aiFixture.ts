...
export function aiFixture() {
  ...
  class Query {
    predicates: Array<(row: Row) => boolean> = []; action = 'read'; value: Row = {}; columns = '*'; orderKey = ''; asc = true; max = Infinity;
    constructor(private table: string) {}
    select(columns = '*') { this.columns = columns; return this; }
    eq(key: string, value: unknown) { this.predicates.push(row => row[key] === value); return this; }
    order(key: string, options: { ascending: boolean }) { this.orderKey = key; this.asc = options.ascending; return this; }
    ...
    execute(single = false): any {
      ...
      rows = rows.slice(0, this.max).map(row => this.columns === '*' ? structuredClone(row) : Object.fromEntries(this.columns.split(',').map(key => [key, structuredClone(row[key])])));
  ...
  const database = { from: (table: string) => new Query(table), rpc: async (name: string, args: Row) => {
  ...
  const generate = async (request: Row) => generateWithModelFallback(['first', 'second'], async model => {
    ...
    let value: unknown;
    if (input.candidates) value = input.candidates.map((candidate: Row) => ({ id: candidate.id, suitabilityScore: 70, confidence: 60, explanation: 'Research evidence needs human confirmation.', strengths: ['Research'], weaknesses: ['Confirm proficiency'], matchingFactors: ['Research'], skillGaps: [], interviewQuestions: ['Describe a research project.'] }));
  ...
  app.use((req: any, res, next) => {
    ...
    if (![student, company, other].includes(id)) return res.status(401).json({ error: 'Test authentication required' });
    req.user = { uid: id, role: id === company ? 'company' : 'student' }; next();
  });
  registerAiWorkforceRoutes(app, generate, () => database as any);
  return { app, state };
}
