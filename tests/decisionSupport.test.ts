...
test('admin profile reviews require specific questions and evidence requests, not learning recommendations', () => {
    ...
    for (const value of [undefined, [], ['Only one'], [1, 'Two', 'Three'], ['Repeated', 'Repeated', 'Third']])
        assert.throws(() => normalizeAiProfileAnalysis({ ...review, verificationQuestions: value }), /assessment/);
...
test('multilingual taxonomy preserves all existing skilled-work options and exact technical skills', () => {
    for (const group of ROLE_GROUPS)
        for (const option of group.roles) {
            for (const locale of ['ko', 'en'] as const)
                assert.ok(classifyRoles([optionLabel(option, locale)]).includes(`role:${option.value}`), `${locale}: ${option.value}`);
        }
    for (const [need, skill] of [['데이터 분석', 'Data Analysis'], ['회계', 'Accounting'], ['번역', 'Translation'], ['고객 지원', 'Customer Service'], ['시장조사', 'Market Research'], ['엑셀', 'Excel'], ['phân tích dữ liệu', 'Data Analysis']])
        assert.equal(skillIsDeclared(need, [skill]), true);
...
test('matching ranks multilingual evidence, keeps missing conditions visible, and omits unrelated filler', () => {
    ...
    assert.equal(result.candidates[0].id, 'relevant');
    ...
    assert.equal(languageMismatch.candidates.length, 0);
    assert.ok(languageMismatch.exclusions[0].reasons.includes('required_language'));
...
test('admin workflow is protected, live-source backed, paginated, reloadable and reports dependency failure', async () => {
    ...
    try {
        ...
        const initial = await get(admin).then(r => r.json());
        ...
        const signed = await fetch(base + file, { headers: { 'x-test-actor': admin } });
        ...
        const response = await post(admin, { role: 'student', profileOwnerId: student, locale: 'ko' });
        assert.equal(response.status, 200);
        assert.ok(state.pendingObserved);
        const body = await response.json();
        assert.ok(body.assessmentId);
        const updated = await get(admin).then(r => r.json());
        assert.equal(updated.members.find((m: any) => m.id === student).ai.state, 'current');
        ...
        companyProfile.data.companyIntroduction = 'Research project team';
        companyProfile.data.requiredSkills = ['Research'];
        const companyAnalysis = await post(admin, { role: 'company', profileOwnerId: company, locale: 'en' });
        assert.equal(companyAnalysis.status, 200);
        ...
        const changed = await get(admin).then(r => r.json());
        assert.equal(changed.members.find((m: any) => m.id === student).ai.state, 'stale');
        const match = await get(admin, `?projectId=${projectId}`).then(r => r.json());
        assert.equal(match.matching.scanned, 301);
        assert.equal(match.matching.candidates.length, 1);
        state.failProvider = true;
        const outage = await post(admin, { role: 'student', profileOwnerId: student });
        assert.equal(outage.status, 502);
        assert.equal((await outage.json()).code, 'AI_GENERATION_FAILED');
        const failed = await get(admin).then(r => r.json());
        assert.equal(failed.members.find((m: any) => m.id === student).ai.state, 'failed');
        ...
        state.failProvider = false;
        state.profileChangedDuringAnalysis = true;
        assert.equal((await post(admin, { role: 'student', profileOwnerId: student })).status, 409);
        assert.equal(state.tables.konexa_ai_assessments.filter(row => row.status === 'pending').length, 0);
        for (let index = 0; index < 510; index++)
            state.tables.app_records.push({ collection_name: 'users', record_id: `extra-${index}`, data: { role: 'student' } });
        const paged = await get(admin).then(r => r.json());
        assert.equal(paged.members.length, 513);
        state.failReads = 'konexa_projects';
        assert.equal((await get(admin)).status, 503);
    }
    finally {
