...
begin
  ...
  perform pg_temp.qa_denied('unfunded_delivery_denied',format('select public.konexa_submit_milestone_v2(%L,%L,%L,ARRAY[]::text[],%L)',student,milestone_id,'[QA] unpaid result','qa-unfunded-submit'),'verified_funding_required');
  ...
  perform pg_temp.qa_denied('empty_delivery_denied',format('select public.konexa_submit_milestone_v2(%L,%L,%L,ARRAY[]::text[],%L)',student,milestone_id,'','qa-empty-submit'),'delivery_evidence_required');
  perform pg_temp.qa_denied('forged_file_denied',format('select public.konexa_submit_milestone_v2(%L,%L,%L,ARRAY[%L]::text[],%L)',student,milestone_id,'[QA] forged attachment',other_student||'/private.pdf','qa-forged-file'),'delivery_file_forbidden');
  perform pg_temp.qa_denied('budget_overallocation_denied',format('select public.konexa_create_milestone_v2(%L,%L,%L::jsonb,%L)',company,contract_id,'{"title":"QA too large","deliverable":"Synthetic research result","amountKrw":400000,"dueAt":"2099-01-01T00:00:00Z"}','qa-overbudget'),'invalid_milestone_budget');
  perform pg_temp.qa_denied('incomplete_project_denied',format('select public.konexa_confirm_completion_v3(%L,%L,%L)',company,contract_id,'qa-incomplete'),'all_deliverables_approval_required');
  update public.konexa_milestones set due_at=clock_timestamp()-interval '1 day' where id=qa.milestone_id;
  perform public.konexa_queue_delivery_reminders_v3();
  perform public.konexa_queue_delivery_reminders_v3();
  perform pg_temp.qa_assert('deadline_reminder_is_idempotent',(select count(*)=1 from public.konexa_notification_outbox o where o.idempotency_key like 'delivery-reminder:'||qa.milestone_id::text||':%'));
  ...
  perform public.konexa_review_delivery_v3(company,milestone_id,'rejected','[QA] Please add research sources.','qa-request-revision');
  ...
  perform public.konexa_review_delivery_v3(company,milestone_id,'rejected','[QA] Please clarify source selection.','qa-request-revision2');
  result := public.konexa_submit_milestone_v2(student,milestone_id,'[QA] Final sourced deliverable',array[]::text[],'qa-submit3');
  perform public.konexa_review_delivery_v3(company,milestone_id,'approved','[QA] Sources and scope checked.','qa-approve');
  perform pg_temp.qa_assert('each_revision_has_notification',(select count(*)=3 from public.konexa_notification_outbox o where o.idempotency_key like 'delivery-reviewed:%' and o.payload->>'milestoneId'=qa.milestone_id::text));
  perform pg_temp.qa_assert('feedback_history_preserved',(select count(*)=3 from public.konexa_milestone_submissions s where s.milestone_id=qa.milestone_id and s.review_feedback is not null));
  ...
  perform pg_temp.qa_denied('review_before_completion_denied',format('select public.konexa_create_review_v2(%L,%L,%L,5,5,5,5,5,%L,%L)',student,relationship_id,contract_id,'[QA] Review content for database validation only','qa-not-complete-review'),'project_completion_required');
  perform pg_temp.qa_denied('unrelated_completion_denied',format('select public.konexa_confirm_completion_v3(%L,%L,%L)',other_student,contract_id,'qa-forbidden-complete'),'completion_forbidden');
  result:=public.konexa_confirm_completion_v3(company,contract_id,'qa-company-complete');
  perform pg_temp.qa_assert('one_party_does_not_issue_passport',result->>'status'='awaiting_other_party' and not exists(select 1 from public.konexa_work_passport_entries w where w.contract_id=qa.contract_id));
  result:=public.konexa_confirm_completion_v3(student,contract_id,'qa-student-complete');
  perform pg_temp.qa_assert('bilateral_completion_issues_passport',result->>'status'='completed' and (select count(*)=1 from public.konexa_work_passport_entries w where w.contract_id=qa.contract_id));
  perform public.konexa_confirm_completion_v3(student,contract_id,'qa-student-complete');
  perform pg_temp.qa_assert('completion_retry_idempotent',(select count(*)=1 from public.konexa_work_passport_entries w where w.contract_id=qa.contract_id));
  perform pg_temp.qa_assert('completion_rpc_service_only',not has_function_privilege('authenticated','public.konexa_confirm_completion_v3(uuid,uuid,text)','execute'));
  ...
  perform pg_temp.qa_denied('student_cannot_resolve_dispute',format('select public.konexa_resolve_dispute_v3(%L,%L,%L,true,%L)',student,result->>'id','[QA] Unauthorized resolution attempt.','qa-student-resolve'),'admin_role_required');
  result:=public.konexa_resolve_dispute_v3(admin_id,(result->>'id')::uuid,'[QA] Submitted records reviewed; both parties confirmed the scope.',true,'qa-admin-resolve');
  perform pg_temp.qa_assert('admin_resolution_restores_evidence_state',exists(select 1 from public.konexa_contracts c where c.id=qa.contract_id and c.status='completed') and exists(select 1 from public.konexa_milestones m where m.id=qa.milestone_id and m.status='approved'));
  perform pg_temp.qa_assert('resolution_does_not_move_money',(select count(*)=1 from public.konexa_payment_orders p where p.contract_id=qa.contract_id) and not exists(select 1 from public.konexa_ledger_transactions l join public.konexa_payment_orders p on p.id=l.payment_order_id where p.contract_id=qa.contract_id));
