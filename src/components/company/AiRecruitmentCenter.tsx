  ...
  return <div className="mx-auto max-w-7xl space-y-6 pb-12">
    ...
    {activeMatch && <div className="grid gap-6 lg:grid-cols-[330px_1fr]">
      ...
      <main className="space-y-6 rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm md:p-8">
        ...
        <section className="rounded-3xl bg-neutral-950 p-5 text-white"><div className="flex items-center justify-between"><h3 className="flex items-center gap-2 text-sm font-black"><Brain className="h-4 w-4" />AI 채용 도우미</h3><span className="text-[9px] text-neutral-400">{chat.model || "Gemini"}</span></div><p className="mt-2 text-xs leading-5 text-neutral-300">대화는 계정에 저장되며 Gemini로 처리됩니다. 개인정보·영업비밀은 입력하지 마세요. 제안은 채용 결정이나 보증이 아닙니다.</p><button onClick={chat.reload} disabled={chatLoading} className="mt-2 text-xs underline">저장된 대화 다시 불러오기</button><div className="mt-4 max-h-64 space-y-3 overflow-y-auto">{chatMessages.map((message) => <div data-no-translate key={message.id} className={`max-w-[88%] whitespace-pre-wrap break-words rounded-2xl p-3 text-sm leading-6 ${message.role === "user" ? "ml-auto bg-white text-neutral-950" : "bg-white/10 text-neutral-100"}`}>{message.content}</div>)}{chatLoading && <div className="text-xs text-neutral-400">응답을 작성하고 있습니다.</div>}</div><div className="mt-4 flex gap-2"><input maxLength={4000} disabled={chatLoading} value={chatInput} onChange={(event) => setChatInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.nativeEvent.isComposing) void sendChat(); }} placeholder="공고 범위나 면접 평가 기준을 물어보세요" className="min-w-0 flex-1 rounded-xl border border-white/15 bg-white/10 px-4 py-3 text-sm text-white outline-none placeholder:text-neutral-500" /><button onClick={sendChat} disabled={chatLoading || !chatInput.trim()} className="rounded-xl bg-white p-3 text-neutral-950 disabled:opacity-40"><Send className="h-4 w-4" /></button></div></section>
      </main>
    </div>}
  </div>;
}
