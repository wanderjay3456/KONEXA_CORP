...
export function criterionLabel(key: string, locale = 'ko') { return labels[key]?.[locale === 'ko' ? 0 : 1] || key; }
export default function MatchCriteria({ criteria }: {
    criteria: Criterion[];
}) {
    const { locale } = useLocale();
    const t = (ko: string, en: string) => locale === 'ko' ? ko : en;
    return <section className="space-y-3">
    <h3 className="text-sm font-bold">{t('공고와 프로필 비교', 'Project-to-profile comparison')}</h3>
    <p className="text-xs leading-5 text-neutral-600">{t('입력 내용의 일치 여부입니다. 역량 검증이나 채용 확률을 뜻하지 않습니다.', 'Compares declared information. This is not verified ability or a hiring probability.')}</p>
    <div className="divide-y divide-neutral-100 rounded-xl border border-neutral-200">
      {criteria.map((item, i) => <div key={`${item.key}-${i}`} className="grid gap-2 p-3 text-sm sm:grid-cols-[1fr_1fr_auto]">
        ...
        <div className="min-w-0 break-words text-neutral-700">{item.declared || t('미입력', 'Not provided')}</div>
        <span className={`self-start whitespace-nowrap rounded-md px-2 py-1 text-xs ${item.status === 'declared' ? 'bg-teal-50 text-teal-800' : item.status === 'conflict' ? 'bg-rose-50 text-rose-800' : 'bg-amber-50 text-amber-900'}`}>{item.status === 'declared' ? t('입력상 일치', 'Declared match') : item.status === 'conflict' ? t('조건 충돌', 'Conflict') : t('확인 필요', 'Verify')}</span>
      </div>)}
    </div>
  </section>;
}
