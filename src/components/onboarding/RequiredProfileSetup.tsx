...
type Field = {
    key: string;
    label: Text;
    type?: "email" | "url" | "number" | "textarea" | "tags";
    choices?: readonly string[];
    required?: boolean;
    max?: number;
};
const tr = (text: readonly string[], locale: Locale) => text[locale === "ko" ? 0 : 1];
...
export default function RequiredProfileSetup({ onComplete, onCancel }: {
    onComplete?: () => void;
    onCancel?: () => void;
}) {
    ...
    async function save(final: boolean, draftOnly = false) {
        ...
        if (!draftOnly && fieldErrors.length) {
        ...
        try {
            ...
            void fetch("/api/gemini/analyze-profile", {
                ...
                await refreshWorkspaceProfile(); }).catch(() => undefined);
            onComplete?.();
        }
        catch {
            setFailure(t("fail"));
    ...
    const fileField = (key: string, label: string, accept: string) => (<label key={key} className={`block rounded-2xl border p-5 ${invalid.includes(key) ? "border-red-500" : "border-[#17342d]/15"}`}>
      ...
      <input aria-label={label} type="file" accept={accept} disabled={busy} onChange={(event) => { const file = event.target.files?.[0]; if (file) {
        setFiles((previous) => ({ ...previous, [key]: file }));
        setInvalid((previous) => previous.filter((item) => item !== key));
    } }} className="mt-3 block w-full text-sm file:mr-3 file:rounded-lg file:border-0 file:bg-[#eef1ed] file:px-3 file:py-2 file:font-semibold"/>
    </label>);
    ...
    <div className="mx-auto max-w-3xl">
      <div className="flex items-center justify-between gap-4">
        ...
        <div role="group" aria-label="Language" className="flex gap-1 rounded-full border border-[#17342d]/15 bg-white p-1">{(["ko", "en"] as const).map((language) => <button type="button" key={language} aria-pressed={locale === language} onClick={() => setLocale(language)} className={`rounded-full px-3 py-1.5 text-xs font-bold uppercase ${locale === language ? "bg-[#17342d] text-white" : ""}`}>{language}</button>)}</div>
      </div>
      ...
      <ol aria-label="Profile progress" className="my-7 grid grid-cols-3 gap-2">{[0, 1, 2].map((index) => <li key={index} aria-current={index === step ? "step" : undefined} className={`rounded-xl px-3 py-3 text-center text-xs font-bold leading-5 ${index <= step ? "bg-[#17342d] text-white" : "bg-[#e7ebe4] text-[#61746c]"}`}>{index + 1}. {locale === "ko" ? copy[isStudent ? "studentSteps" : "companySteps"][index] : ["About you", "Work preferences", "Documents"][index]}</li>)}</ol>
          ...
          <div className="grid gap-5 sm:grid-cols-2">{fields.map((field) => {
            ...
            return <div key={field.key} className={field.type === "textarea" || field.type === "tags" ? "sm:col-span-2" : ""}>
              ...
              {field.type === "tags" ? <div>
                ...
                <input id={id} className={inputClass} placeholder={t("add")} onKeyDown={(event) => { if (event.key === "Enter") {
                    event.preventDefault();
                    const input = event.currentTarget;
                    const value = input.value.trim();
                    if (value)
                        update(field.key, Array.from(new Set([...(draft[field.key] || []), value])));
                    input.value = "";
                } }}/>
