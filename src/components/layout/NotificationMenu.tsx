...
export default function NotificationMenu({ onNavigate }: NotificationMenuProps) {
    ...
    const unreadLabel = { ko: `읽지 않은 알림 ${unreadNotificationCount}개`, en: `${unreadNotificationCount} unread notifications` }[locale];
    ...
    return (<div className="relative">
      ...
      {open && (<div className="fixed inset-x-3 top-20 z-50 overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-2xl sm:absolute sm:inset-auto sm:right-0 sm:top-12 sm:w-[380px]">
          ...
          <div className="max-h-[min(60vh,520px)] overflow-y-auto">
            {notifications.length === 0 ? (<div className="px-6 py-12 text-center"><Bell className="mx-auto h-6 w-6 text-neutral-400"/><p className="mt-3 text-sm font-bold text-neutral-800">아직 알림이 없습니다.</p><p className="mt-1 text-xs text-neutral-600">지원·계약·결제·검증 상태가 변경되면 표시됩니다.</p></div>) : notifications.slice(0, 20).map((notification) => {
                const Icon = kindIcons[notification.kind] || Bell;
                const unread = !notification.readAt;
                return (<button type="button" disabled={saving} key={notification.id} onClick={() => void selectNotification(notification.id, notification.entityType === 'coordination_action' ? 'coordination' : notification.actionTab)} className={`flex w-full gap-3 border-b border-neutral-100 px-4 py-3 text-left transition hover:bg-neutral-50 disabled:opacity-50 ${unread ? "bg-blue-50/50" : "bg-white"}`}>
