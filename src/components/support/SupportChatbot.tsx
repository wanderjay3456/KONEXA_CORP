...
export default function SupportChatbot() {
    ...
    const submit = async (e: FormEvent) => {
        ...
        try {
            ...
            const articleIds = Array.isArray(body.articles) ? body.articles.map((item: {
                id: string;
            }) => item.id).filter((entry: string) => SUPPORT_ARTICLES.some(article => article.id === entry)) : [];
            if (run === generation.current)
                setTurns(items => items.map(turn => turn.id === id ? { ...turn, articleIds, fallback: !articleIds.length, offline: body.source === 'reviewed_help_fallback' } : turn));
        }
        catch {
            const articleIds = findSupportArticles(question).filter(item => item.score >= 7).map(item => item.article.id);
            if (run === generation.current)
                setTurns(items => items.map(turn => turn.id === id ? { ...turn, articleIds, fallback: !articleIds.length, offline: true } : turn));
        }
        finally {
    ...
    {open && <section role="dialog" aria-label={t.open} className={`fixed inset-x-3 flex flex-col overflow-hidden rounded-3xl border border-neutral-200 bg-white text-neutral-900 shadow-2xl sm:absolute sm:inset-x-auto sm:bottom-16 sm:right-0 sm:w-[420px] ${inWorkspace ? 'bottom-40 max-h-[calc(100dvh-11rem)]' : 'bottom-20 max-h-[calc(100dvh-6.5rem)]'}`}>
      ...
      <div className="min-h-24 flex-1 space-y-4 overflow-y-auto overscroll-contain px-4 py-4" aria-live="polite" aria-relevant="additions text">
        ...
        {turns.map(turn => <div key={turn.id} className="space-y-3">
          ...
          {turn.articleIds.map(id => {
                    const article = supportArticleView(id, locale);
                    return article && <div key={id} className="rounded-2xl border border-neutral-200 bg-neutral-50 px-4 py-3">
      ...
      <div className="shrink-0 border-t border-neutral-100 px-4 py-3">
        ...
        } }}/><button type="submit" aria-label={t.send} disabled={busy || !input.trim()} className="rounded-xl bg-teal-800 p-3 text-white disabled:opacity-40"><Send size={18}/></button></form>
