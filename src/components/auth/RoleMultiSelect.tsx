...
const interfaceCopy = {
  ...
  vi: { all: "Tất cả", selected: "đã chọn", empty: "Bạn có thể chọn nhiều lĩnh vực quan tâm." },
} as const;
...
export default function RoleMultiSelect({ locale, selected, onChange, legend, hint }: RoleMultiSelectProps) {
  ...
  const roles = useMemo(
    () => (activeGroup === "all" ? ROLE_GROUPS.flatMap((group) => group.roles) : ROLE_GROUPS.find((group) => group.id === activeGroup)?.roles || []),
    [activeGroup],
  );
      ...
      <div role="group" className="mt-4 flex gap-2 overflow-x-auto pb-2" aria-label={legend}>
        <button
          ...
          className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-bold transition ${
            activeGroup === "all" ? "border-[#4361ee] bg-[#4361ee] text-white" : "border-[#17342d]/15 bg-white text-[#49655e]"
          }`}
        >
          {t.all}
        </button>
        {ROLE_GROUPS.map((group) => (
          <button
            ...
            className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-bold transition ${
              activeGroup === group.id ? "border-[#4361ee] bg-[#4361ee] text-white" : "border-[#17342d]/15 bg-white text-[#49655e]"
            }`}
          >
            {group.label[locale]}
          </button>
          ...
          {roles.map((role) => {
            ...
            return (
              <button
                ...
                className={`rounded-full border px-3 py-2 text-left text-xs font-bold leading-5 transition ${
                  isSelected
