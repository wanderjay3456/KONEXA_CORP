...
import {
  createHiringOffer, createProjectContract, loadTrustSnapshot, recordConsent,
...
export default function TrustOperationsCenter({ initialSection = 'contracts' }: { initialSection?: 'contracts' | 'coordination' }) {
  ...
  const [materialsDue,setMaterialsDue]=useState('');
  ...
  const [reviewRatings, setReviewRatings] = useState({ overall: 0, quality: 0, communication: 0, reliability: 0, scope: 0 });
  ...
  const relationshipLabel = (item: Record<string,unknown>) => projects.find(p=>p.id===item.projectId)?.title || `${activeRole===UserRole.STUDENT?'기업':'인재'} ${asText(activeRole===UserRole.STUDENT?item.companyId:item.talentId).slice(0,8)}`;
  useEffect(()=>{
    const project=projects.find(p=>p.id===selectedIntro?.projectId);
    if(project){setJobTitle(project.title);setScope(project.requirements.join('\n'));setWeeklyHours(project.hoursPerWeek||0);setMonthlyAmount((project.weeklyPayKrw||0)*Math.min(project.durationWeeks||4,4));}
  ...
  const handleContract = async () => {
    ...
    try {
      const contractId = await createProjectContract({
        ...
        weeklyHours,
        revisions,
        clientMaterialsDue: materialsDue,
        reviewDays,
        changeRequestRateKrw: changeRate,
  ...
  const handleReview = async () => {
    ...
    const contract = snapshot.contracts.find((item) => asText(item.relationshipId) === selectedIntro.id && item.status === 'completed');
    if (!contract) return error('프로젝트 종료 확인 필요', '결과물 검수와 양측 완료 확인이 끝난 후 최종 리뷰를 작성할 수 있습니다.');
    try {
      ...
      setReviewComment("");
      success("리뷰가 비공개로 저장되었습니다", "양측 제출과 관리자 내용 검토가 완료된 후 공개됩니다.");
      await refresh();
    } catch (cause) {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-neutral-50 p-4 md:p-7">
      <div className="mx-auto max-w-7xl space-y-6 pb-14">
        {activeRole===UserRole.ADMIN&&<AdminCaseActions snapshot={snapshot} refresh={refresh}/>}
        ...
        {activeSection === "contracts" && (
          <div className="grid gap-6 lg:grid-cols-2">
            <DeliveryWorkspace snapshot={snapshot} refresh={refresh} />
            {activeRole === UserRole.COMPANY && (
              <section className="rounded-3xl border border-neutral-200 bg-white p-6">
                <div className="flex items-center gap-2"><FileSignature className="h-5 w-5" /><h2 className="font-black">프로젝트 계약 발행</h2></div>
                <div className="mt-4 space-y-3">
                  <select aria-label="계약할 프로젝트 선택" value={selectedRelationship} onChange={(e) => setSelectedRelationship(e.target.value)} className="h-11 w-full rounded-xl border border-neutral-200 bg-neutral-50 px-4 text-sm"><option value="">승인된 지원·소개관계 선택</option>{snapshot.introductions.filter(item=>!['completed','cancelled'].includes(asText(item.status))).map((item) => <option key={item.id} value={item.id}>{relationshipLabel(item)}</option>)}</select>
                  <label className="block text-sm">계약 제목<input value={jobTitle} onChange={e=>setJobTitle(e.target.value)} className="mt-2 w-full rounded-xl border p-3" /></label>
                  <label className="block text-sm">포함 결과물<textarea value={scope} onChange={(e) => setScope(e.target.value)} rows={3} className="mt-2 w-full rounded-xl border border-neutral-200 bg-neutral-50 p-4 text-base leading-7" /></label>
                  <label className="block text-sm">포함하지 않는 업무<textarea value={excludedWork} onChange={(e) => setExcludedWork(e.target.value)} rows={3} className="mt-2 w-full rounded-xl border border-neutral-200 bg-neutral-50 p-4 text-base leading-7" /></label>
                  <label className="block text-sm">월 프로젝트 금액(KRW)<input type="number" min={1000} value={monthlyAmount||''} onChange={(e) => setMonthlyAmount(Number(e.target.value))} className="mt-2 h-12 w-full rounded-xl border border-neutral-200 bg-neutral-50 px-4 text-base" /></label>
                  <div className="grid gap-3 sm:grid-cols-2">{[
                    ['주당 예상시간',weeklyHours,setWeeklyHours,1,80],['포함 수정 횟수',revisions,setRevisions,0,20],['검수 기간(일)',reviewDays,setReviewDays,1,30],['추가업무 단가(KRW)',changeRate,setChangeRate,0,100000000],
                  ].map(([label,value,setValue,min,max])=><label key={String(label)} className="text-sm">{String(label)}<input type="number" min={Number(min)} max={Number(max)} value={Number(value)||''} onChange={e=>(setValue as (n:number)=>void)(Number(e.target.value))} className="mt-2 w-full rounded-xl border p-3" /></label>)}</div>
                  <label className="block text-sm">기업이 제공할 자료와 기한<input value={materialsDue} onChange={e=>setMaterialsDue(e.target.value)} className="mt-2 w-full rounded-xl border p-3" /></label>
                  <p className="rounded-xl bg-neutral-50 p-3 text-sm leading-6 text-neutral-600">공고에서 가져온 결과물·시간·금액을 확인해 주세요. 추가업무와 일정 변경은 양측이 합의한 후 계약에 반영해야 합니다.</p>
        ...
        {activeSection === "reviews" && (
          <div className="grid gap-6 lg:grid-cols-[1.05fr_.95fr]">
            {activeRole!==UserRole.ADMIN&&<section className="rounded-3xl border border-neutral-200 bg-white p-6">
              <div className="flex items-start justify-between gap-4">
                <div><div className="flex items-center gap-2"><Star className="h-5 w-5 fill-amber-400 text-amber-400" /><h2 className="font-black">검증 거래 상호 리뷰</h2></div><p className="mt-2 text-sm leading-6 text-neutral-600">대금 확보와 결과물 검수, 양측 완료 확인이 끝난 프로젝트를 평가합니다. 먼저 제출한 평가는 상대방에게 보이지 않으며 양측 제출과 관리자 내용 검토가 완료된 후 공개됩니다.</p></div>
              ...
              <div className="mt-6 space-y-4">
                <label className="block text-[11px] font-bold text-neutral-600">평가할 거래
                  <select value={selectedRelationship} onChange={(event) => setSelectedRelationship(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-neutral-200 bg-neutral-50 px-4 text-xs">
                    ...
                    {snapshot.introductions.filter(item=>snapshot.contracts.some(c=>c.relationshipId===item.id&&c.status==='completed')&&!snapshot.reviews.some(r=>r.relationshipId===item.id&&r.reviewerId===currentUser?.uid)).map((item) => <option key={item.id} value={item.id}>{relationshipLabel(item)}</option>)}
                  </select>
                </label>
                {([
                ...
                <button onClick={() => void handleReview()} disabled={reviewComment.trim().length < 20 || Object.values(reviewRatings).some(r=>Number(r)<1)} className="h-12 w-full rounded-xl bg-neutral-950 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-35">리뷰 봉인 제출</button>
              </div>
            </section>}

            <section className="rounded-3xl border border-neutral-200 bg-white p-6">
              <div className="flex items-center justify-between"><h2 className="font-black">리뷰 기록</h2><span className="text-[10px] font-bold text-neutral-400">{snapshot.reviews.length}건</span></div>
              <div className="mt-5 space-y-3">
                {snapshot.reviews.map((review) => {
                  ...
                  return <div key={review.id} className="rounded-2xl border border-neutral-200 p-4">
                    <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-1">{[1, 2, 3, 4, 5].map((rating) => <Star key={rating} className={`h-3.5 w-3.5 ${rating <= Number(review.overallRating || 0) ? "fill-amber-400 text-amber-400" : "text-neutral-200"}`} />)}</div><StatusPill good={published}>{published ? "양측 공개" : review.moderationStatus==='rejected' ? "게시 제한" : "비공개 · 제출·검토 대기"}</StatusPill></div>
