...
const copy = {
  ...
  vi: { title:'Sản phẩm bàn giao và hoàn thành dự án', intro:'Thống nhất kết quả hằng tuần, nộp minh chứng và lưu từng lần đánh giá. Hoàn thành dự án không có nghĩa là đã thanh toán cho nhân tài.', contract:'Hợp đồng dự án', select:'Chọn hợp đồng', empty:'Chưa có hợp đồng dự án. Hãy duyệt ứng viên và chuẩn bị hợp đồng trước.', stage:'Trạng thái', create:'Thêm mốc công việc', name:'Tên mốc công việc', result:'Sản phẩm cần bàn giao', due:'Hạn nộp', amount:'Giá trị mốc công việc (KRW)', saved:'Đã lưu. Bên còn lại sẽ nhận thông báo.', failed:'Không thể lưu. Nội dung vẫn được giữ; hãy kiểm tra thông báo và thử lại.', loading:'Đang tải dữ liệu…', refresh:'Tải lại dữ liệu', noMilestones:'Chưa có mốc công việc.', notes:'Kết quả hoặc ghi chú bàn giao', file:'Đính kèm sản phẩm (tùy chọn, tối đa 50 MB)', submit:'Gửi để đánh giá', history:'Lịch sử bàn giao', feedback:'Nhận xét (ít nhất 10 ký tự)', approve:'Duyệt sản phẩm', revise:'Yêu cầu chỉnh sửa', finish:'Xác nhận hoàn thành dự án', confirm:'Tôi đã kiểm tra mọi sản phẩm và xác nhận dự án hoàn thành.', waiting:'Đã lưu xác nhận của bạn. Đang chờ bên còn lại.', complete:'Hai bên đã xác nhận hoàn thành. Minh chứng đã được lưu vào Work Passport.', gate:'Chỉ bắt đầu công việc sau khi xác minh nguồn tiền. Tất cả sản phẩm phải được duyệt và tranh chấp phải được giải quyết trước khi hoàn thành.', signedGate:'Có thể thêm mốc công việc sau khi ký hợp đồng.', blank:'Đính kèm tệp hoặc mô tả kết quả bằng ít nhất 10 ký tự.', unavailable:'Không thể mở tệp. Hãy tải lại hoặc liên hệ hỗ trợ.', pending:'Đang lưu…', noHistory:'Chưa có minh chứng được nộp.', consent:'Bước này chỉ ghi nhận hoàn thành dự án, không phải chữ ký hợp đồng điện tử.', passport:'Lịch sử dự án đã xác minh', approved:'Đã duyệt', rejected:'Cần chỉnh sửa', submitted:'Chờ đánh giá', scheduled:'Đã lên lịch', active:'Đang thực hiện', funded:'Đã xác nhận nguồn tiền', issued:'Đã tạo hợp đồng', completed:'Hoàn thành', cancelled:'Đã hủy' },
};
...
type Submission = { id:string; version:number; notes:string; created_at:string; review_feedback?:string; review_decision?:string; files:Array<{name:string;url:string|null}> };

export default function DeliveryWorkspace({ snapshot, refresh }: { snapshot: TrustSnapshot; refresh:()=>Promise<void> }) {
  const { currentUser } = useApp(); const { locale } = useLocale(); const t=copy[locale];
  ...
  const [notes,setNotes]=useState<Record<string,string>>({}); const [files,setFiles]=useState<Record<string,File|null>>({});
  const uploaded=useRef<Record<string,string>>({}); const keys=useRef(new Map<string,string>());
  const [feedback,setFeedback]=useState<Record<string,string>>({}); const [history,setHistory]=useState<Record<string,Submission[]>>({});
  const [historyBusy,setHistoryBusy]=useState(''); const [confirmed,setConfirmed]=useState(false);
  ...
  const company=contract?.companyId===currentUser?.uid; const student=contract?.talentId===currentUser?.uid;
  ...
  async function run(action:()=>Promise<void>) {
    ...
    try { await action(); await refresh(); setMessage(t.saved); }
    catch(cause){setMessage(`${t.failed} ${cause instanceof Error?cause.message:''}`);}finally{setBusy(false);}
  }
  async function loadHistory(id:string) {
    setHistoryBusy(id);
    try { const response=await fetch(`/api/v2/milestones/${id}/submissions`,{cache:'no-store'});const value=await response.json();
      if(!response.ok)throw new Error(value.error?.message||t.failed);setHistory(h=>({...h,[id]:value.data}));
    }catch(cause){setMessage(cause instanceof Error?cause.message:t.failed);}finally{setHistoryBusy('');}
  }
  async function submit(id:string) {
    await run(async()=>{
      if(!uploaded.current[id]&&files[id]) uploaded.current[id]=await uploadPrivateFile('project-deliverables',currentUser!.uid,files[id]!);
      const paths=uploaded.current[id]?[uploaded.current[id]]:[];
      if((notes[id]||'').trim().length<10&&!paths.length)throw new Error(t.blank);
      await command(`/api/v2/milestones/${id}/submissions`,{notes:notes[id]||'',storagePaths:paths});
      delete uploaded.current[id];setFiles(f=>({...f,[id]:null}));setNotes(n=>({...n,[id]:''}));await loadHistory(id);
    });
  }
