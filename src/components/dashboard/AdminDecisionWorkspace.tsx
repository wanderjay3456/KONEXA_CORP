      ...
      <div className="grid items-start gap-5 lg:grid-cols-[310px_minmax(0,1fr)]">
        ...
        {member && <article className={`${box} min-w-0 space-y-5`}>
          ...
          <section className="space-y-4 border-t border-neutral-200 pt-5">
            ...
            {member.ai.gaps && <div className="rounded-xl bg-amber-50 p-4 text-amber-950"><h5 className="text-sm font-semibold">{t('추가 확인이 필요한 정보', 'Details to clarify', 'Thông tin cần làm rõ')}</h5><p className="mt-2 text-sm leading-7">{member.ai.gaps}</p></div>}
