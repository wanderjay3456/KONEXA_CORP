      ...
      <div className="grid items-start gap-5 lg:grid-cols-[310px_minmax(0,1fr)]">
        ...
        {member && <article className={`${box} min-w-0 space-y-5`}>
          ...
          <section className="space-y-4 border-t border-neutral-200 pt-5">
            <h4 className="text-sm font-bold">{t('AI 참고 요약', 'AI advisory summary', 'Tóm tắt AI tham khảo')} · {aiState(member.ai.state)}</h4>
            <p className="text-xs leading-6 text-neutral-600">{t('AI가 본인 입력 내용을 요약하고 확인 질문을 제안합니다. 위 ‘다음 확인 사항’의 미입력 항목은 실제 저장값으로 판단하며, AI 문장으로 추가하거나 변경하지 않습니다. 요약은 실제 역량 검증 결과가 아닙니다.', 'AI summarizes self-reported information and suggests review questions. Missing fields in Next verification steps come only from stored values, never AI wording. This summary is not proof of competence.', 'AI tóm tắt thông tin tự khai và gợi ý câu hỏi xác minh. Mục còn thiếu ở Bước xác minh tiếp theo chỉ dựa trên dữ liệu đã lưu, không dựa trên lời AI. Tóm tắt không chứng minh năng lực.')}</p>
            ...
            {member.ai.verificationQuestions.length > 0 && <div><h5 className="text-sm font-semibold">{t('면담에서 확인할 질문', 'Questions for the review conversation', 'Câu hỏi khi trao đổi')}</h5><ol className="mt-2 list-decimal space-y-2 pl-5 text-sm leading-6">{member.ai.verificationQuestions.map((question, i) => <li key={i}>{question}</li>)}</ol></div>}
            {member.ai.evidenceRequests.length > 0 && <div><h5 className="text-sm font-semibold">{t('확인할 작업물·업무 자료', 'Work samples or scope documents to review', 'Sản phẩm hoặc tài liệu phạm vi cần xem')}</h5><ul className="mt-2 list-disc space-y-2 pl-5 text-sm leading-6">{member.ai.evidenceRequests.map((request, i) => <li key={i}>{request}</li>)}</ul></div>}
            {member.ai.strength && !member.ai.verificationQuestions.length && <p className="text-sm leading-6 text-neutral-600">{t('이전 분석에는 면담 질문이 없습니다. 다시 분석하면 확인 질문과 요청 자료를 함께 저장합니다.', 'This earlier analysis has no review questions. Generate a new review to save questions and evidence requests.', 'Phân tích trước chưa có câu hỏi. Phân tích lại để lưu câu hỏi và tài liệu cần xác minh.')}</p>}
            {!member.ai.strength && <p className="text-sm text-neutral-600">{t('저장된 AI 분석이 없습니다. 위 실제 정보로 검토하거나 분석을 요청하세요.', 'No saved AI analysis. Review the actual evidence above or request analysis.', 'Chưa có phân tích AI. Xem thông tin thực ở trên hoặc yêu cầu phân tích.')}</p>}
          </section>
        </article>}
      </div>
