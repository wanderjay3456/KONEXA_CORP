...
export default function PublicOpportunityPreview({ locale, onLogin, onStudent, onCompany }: PublicOpportunityPreviewProps) {
    ...
    useEffect(() => {
        ...
        void fetch("/api/public/projects", { signal: controller.signal })
            ...
            if (!response.ok)
                throw new Error(payload?.error?.message || "Public opportunities are unavailable");
    ...
    return (<section id="opportunities" className="bg-[#f7f6f1] px-5 py-24 sm:px-8 lg:py-32">
      <div className="mx-auto max-w-7xl">
        ...
        {loading ? (<div role="status" aria-live="polite" className="mt-12 grid gap-4 md:grid-cols-3">
            <span className="sr-only">{t.loading}</span>
            {[0, 1, 2].map((item) => <div key={item} className="h-64 animate-pulse rounded-[1.7rem] bg-[#17342d]/5"/>)}
          </div>) : failed ? (<div role="status" className="mt-12 rounded-3xl border border-[#17342d]/15 bg-white p-8 text-center">
          ...
          </div>) : projects.length > 0 ? (<div className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {projects.map((project) => {
                const requiredSkills = [...new Set([...(project.requirements || []), ...(project.tags || [])])].slice(0, 4);
                return (<article key={project.id} className="group flex min-h-72 flex-col rounded-[1.7rem] border border-[#17342d]/10 bg-white p-6 shadow-[0_18px_60px_rgba(23,52,45,.06)] transition hover:-translate-y-1 hover:shadow-[0_24px_70px_rgba(23,52,45,.11)]">
          ...
          </div>) : (<div className="mt-12 rounded-[2rem] border border-dashed border-[#17342d]/20 bg-white/70 px-6 py-12 text-center sm:px-10">
