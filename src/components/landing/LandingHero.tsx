      ...
      {activeRegisterRole ? <motion.main key="registration" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="min-h-screen bg-[#f7f6f1] text-neutral-900">
        ...
        <main>
          <section ref={heroRef} className="relative flex min-h-[880px] items-center px-5 pb-24 pt-32 sm:px-8 lg:min-h-screen">
            ...
            <div className="relative z-10 mx-auto grid w-full max-w-7xl items-center gap-14 lg:grid-cols-[1.08fr_.92fr]">
              <motion.div style={{ y: heroCopyY, opacity: heroCopyOpacity }}>
                <motion.div initial={reduced ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-7 inline-flex items-center gap-2 rounded-full border border-[#17342d]/10 bg-white/75 px-3 py-1.5 text-xs font-black uppercase tracking-[.1em] text-[#294d44]"><Sparkles className="h-3.5 w-3.5 text-[#4361ee]" />{t.badge}</motion.div>
          ...
          <section id="reviews" className="bg-[#17342d] px-5 py-24 text-white sm:px-8 lg:py-36"><div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-[.85fr_1.15fr] lg:items-center">
            <Reveal><div className="font-mono text-xs font-bold tracking-[.16em] text-[#b9f4d0]">03 / {t.reviewEyebrow}</div><h2 className="mt-5 whitespace-pre-line font-display text-4xl font-bold leading-[1.03] tracking-[-.05em] sm:text-6xl">{t.reviewTitle}</h2><p className="mt-7 max-w-xl text-base leading-8 text-white/80">{t.reviewLead}</p><div className="mt-8 grid grid-cols-2 gap-2">{t.safeguards.map(item => <div key={item} className="flex items-center gap-2 rounded-full border border-white/20 px-3 py-2 text-xs font-bold text-white/85"><Check className="h-3 w-3 text-[#b9f4d0]" />{item}</div>)}</div></Reveal>
