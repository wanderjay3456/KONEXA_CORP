...
import { BadgeCheck, BriefcaseBusiness, Building2, Check, CircleDollarSign, FileSignature, ShieldCheck, Star, UserRoundCheck, } from "lucide-react";
...
function StageCopy({ stage, index, progress }: {
    stage: JourneyStage;
    index: number;
    ...
    return (<motion.article style={{ opacity, y }} className="cinematic-stage" aria-hidden="true">
...
function ProofCard({ stage, index, progress }: {
    stage: JourneyStage;
    index: number;
    progress: MotionValue<number>;
    key?: string;
    ...
    const Icon = icons[index];
    return (<motion.div style={{ opacity, y, scale }} className={`cinematic-proof-card cinematic-proof-card-${index + 1}`}>
...
export default function CinematicTrustJourney({ locale }: {
    locale: Locale;
}) {
    ...
    return (<section ref={sectionRef} className={`cinematic-journey ${reduced ? "cinematic-journey-reduced" : ""}`} aria-label={t.intro}>
