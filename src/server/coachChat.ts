...
export function parseCoachInput(body: any) {
  if (!Array.isArray(body?.messages) || !body.messages.length || body.messages.length > 30) throw new Error('INVALID_MESSAGES');
  const messages = body.messages.map((item: any) => {
    if (!['user', 'assistant'].includes(item?.role) || typeof item.content !== 'string' || !item.content.trim() || item.content.length > 8000) throw new Error('INVALID_MESSAGE');
    return { role: item.role as 'user' | 'assistant', content: item.content.trim() };
  });
  if (messages.at(-1)?.role !== 'user' || messages.reduce((sum: number, item: any) => sum + item.content.length, 0) > 24000) throw new Error('INVALID_CONVERSATION');
  const contextKey = typeof body.contextKey === 'string' ? body.contextKey : 'career';
  if (!/^[a-z0-9:_-]{1,180}$/i.test(contextKey)) throw new Error('INVALID_CONTEXT');
  const id = body.requestId || crypto.randomUUID();
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id)) throw new Error('INVALID_REQUEST_ID');
  const locale = ['ko', 'en', 'vi'].includes(body.locale) ? body.locale : 'en';
...
export function registerCoachChatRoutes(app: Express) {
  app.get('/api/gemini/chat-history', async (req: AuthenticatedRequest, res) => {
    ...
    if (!/^[a-z0-9:_-]{1,180}$/i.test(contextKey)) { res.status(400).json({ error: 'Invalid conversation' }); return; }
    try {
    ...
    } catch { res.status(503).json({ error: 'Saved conversation is temporarily unavailable. Please retry.' }); }
  });
  app.get('/api/gemini/chat/:id', async (req: AuthenticatedRequest, res) => {
    ...
    try {
      ...
      res.json(data);
    } catch { res.status(503).json({ error: 'Saved conversation is temporarily unavailable.' }); }
  });
  app.post('/api/gemini/chat', async (req: AuthenticatedRequest, res) => {
    res.setHeader('Cache-Control', 'no-store');
    let input: ReturnType<typeof parseCoachInput>;
    try { input = parseCoachInput(req.body); }
    catch { res.status(400).json({ error: 'Send a non-empty question; keep the conversation below 24,000 characters.', code: 'INVALID_INPUT' }); return; }
    ...
    try {
      ...
      if (existing?.status === 'completed') { res.json({ reply: existing.result.reply, model: existing.model, generationId: id, restored: true }); return; }
      if (existing) { res.status(409).json({ error: 'This request is already recorded. Reload the conversation before retrying.', code: 'REQUEST_ALREADY_RECORDED' }); return; }
      ...
      if (insertError) {
        if (insertError.code === '23505') { res.status(409).json({ error: 'This request is already processing.' }); return; }
        throw insertError;
      }
      created = true;
      ...
      const { response, model } = await generateGeminiContent({
        ...
        config: { maxOutputTokens: 1600, systemInstruction: `You are KONEXA's career and project preparation assistant. Answer in ${locale === 'ko' ? 'Korean' : locale === 'vi' ? 'Vietnamese' : 'English'}. The authenticated role is ${req.user!.role}; conversation topic: ${contextKey}.
...
Owned project evidence, if any: ${JSON.stringify(projectEvidence)}` },
      ...
      if (!reply || reply.length > 30000) throw new Error('INVALID_REPLY');
      ...
      if (created) await getSupabaseAdmin().from('konexa_ai_generations').update({ status: 'failed', completed_at: new Date().toISOString() }).eq('id', id).eq('user_id', req.user!.uid);
