...
export function validateSupportRouting(value: any) {
    if (!Array.isArray(value?.articleIds) || value.articleIds.length > 3 || safeSupportIds(value.articleIds).length !== value.articleIds.length)
        throw new Error('Invalid structured help routing');
}
export function parseSupportRequest(body: unknown) {
    const value = body as Record<string, unknown> | null;
    if (!value || !['ko', 'en'].includes(String(value.locale)))
        throw new Error('INVALID_LOCALE');
    if (typeof value.question !== 'string' || !value.question.trim() || value.question.length > 1000)
        throw new Error('INVALID_QUESTION');
    const articleId = typeof value.articleId === 'string' ? value.articleId : '';
    if (articleId && !safeSupportIds([articleId]).length)
...
export function registerSupportRoutes(app: Express, getDatabase = getSupabaseAdmin) {
    ...
    app.get('/api/public/support', (_req, res) => {
        ...
        res.json({ knowledgeVersion: SUPPORT_KNOWLEDGE_VERSION, articleCount: SUPPORT_ARTICLES.length, locales: ['ko', 'en'] });
    });
    app.post('/api/public/support/chat', limiter, async (req, res) => {
        ...
        try {
            ...
            res.status(400).json({ code: 'INVALID_INPUT', error: 'Provide a question of 1–1000 characters and a supported language.' });
            return;
        }
        const { question, locale, articleId } = input;
        if (articleId) {
            res.json(responseBody([articleId], locale, 'reviewed_help'));
            return;
        }
        ...
        if (/(?:api[_ -]?key|password|비밀번호|인증코드|mật khẩu)\s*[:=]\s*\S+|\b\d{6}[- ]?\d{7}\b|\b(?:sk-|AIza)[A-Za-z0-9_-]{15,}/i.test(question)) {
            res.json(responseBody(['ai-coach', 'privacy'], locale, 'privacy_guard'));
            return;
        }
        ...
        if (matches[0]?.score >= 12 || !process.env.GEMINI_API_KEY) {
            res.json(responseBody(fallbackIds, locale, 'reviewed_help'));
            return;
        }
        ...
        try {
            ...
            if (budgetError || (count || 0) >= 100)
                throw new Error('HELP_AI_BUDGET_OR_STORAGE_UNAVAILABLE');
            ...
            const { response, model } = await generateGeminiContent({
                ...
                config: { responseMimeType: 'application/json', maxOutputTokens: 1024, httpOptions: { timeout: 12000 },
