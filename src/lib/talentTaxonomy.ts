...
export const ROLE_GROUPS: RoleGroup[] = [
    {
        id: "digital",
        label: { ko: "디지털·데이터", en: "Digital & Data" },
        roles: [
    ...
    {
        id: "engineering",
        label: { ko: "공학·제조", en: "Engineering & Manufacturing" },
        roles: [
    ...
    {
        id: "business",
        label: { ko: "비즈니스·무역", en: "Business & Trade" },
        roles: [
    ...
    {
        id: "creative",
        label: { ko: "디자인·콘텐츠", en: "Design & Content" },
        roles: [
    ...
    {
        id: "science",
        label: { ko: "과학·교육·헬스", en: "Science, Education & Health" },
        roles: [
    ...
    {
        id: "built_service",
        label: { ko: "건설·서비스·숙련기술", en: "Built Environment, Service & Skilled Work" },
        roles: [
...
export function roleLabel(id: string, locale = 'ko') {
    if (id.startsWith('role:')) {
        ...
        if (option)
            return optionLabel(option, locale as Locale);
    }
    const row = ROLE_FAMILIES.find(row => row[0] === id);
    return row ? row[locale === 'ko' ? 2 : 1] : id;
}
