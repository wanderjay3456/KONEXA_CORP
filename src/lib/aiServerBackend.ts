...
export function registerAiWorkforceRoutes(app: any, generateGeminiContent: GenerateGeminiContent) {
  ...
  app.post("/api/ai/resume-review", async (req: any, res: any) => {
    try {
      ...
      const { data: profileRow, error } = await getSupabaseAdmin().from("app_records").select("data").eq("collection_name", "student_profiles").eq("record_id", req.user.uid).maybeSingle();
