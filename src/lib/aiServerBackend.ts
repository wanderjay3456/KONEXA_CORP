...
type GenerateGeminiContent = (request: Record<string, any>, models?: string[]) => Promise<{
    response: any;
    model: string;
}>;
...
async function failAssessment(id: string | undefined, getDatabase = getSupabaseAdmin) {
    ...
    try {
        ...
        console.error('[KONEXA] Assessment failure state could not be saved', { assessmentId: id });
    }
}
export function registerAiWorkforceRoutes(app: any, generateGeminiContent: GenerateGeminiContent, getDatabase = getSupabaseAdmin) {
    app.get('/api/ai/assessments', async (req: any, res: any) => {
        ...
        if (!['student_career_roadmap', 'resume_evidence_review', 'pdf_evidence_extraction', 'talent_project_matching'].includes(type) || !/^[0-9a-f-]{36}$/i.test(entityId))
            return res.status(400).json({ error: 'Invalid assessment query' });
        try {
            ...
            if (type === 'talent_project_matching') {
                ...
                if (error)
                    throw error;
                if (req.user.role !== 'admin' && (company?.data?.verified !== true || company?.data?.verifiedStatus !== 'Verified'))
                    return res.status(403).json({ error: 'Business verification is required' });
                let projectQuery = db.from('konexa_projects').select('*').eq('id', entityId);
                if (req.user.role !== 'admin')
                    projectQuery = projectQuery.eq('company_id', req.user.uid);
            ...
            if (type === 'talent_project_matching' && data?.length) {
                ...
                for (const row of data)
                    row.result = { ...row.result, matches: (row.result?.matches || []).filter((match: any) => eligible.has(match.id)), historical: true, stale: row.prompt_version !== MATCHING_VERSION || row.result?.evidenceFingerprint !== fingerprint };
            }
            res.json({ data: data || [] });
        }
        catch {
            res.status(503).json({ error: 'Saved analysis is temporarily unavailable. Please retry.' });
        }
    });
    app.post("/api/ai/diagnostics", async (req: any, res: any) => {
        ...
        try {
            ...
            if (parsed.status !== "ok")
                throw new Error("Unexpected provider verification response");
            checks.push({
    ...
    app.post("/api/ai/matching", async (req: any, res: any) => {
        ...
        try {
            ...
            if (project.status !== 'open')
                return res.status(409).json({ error: 'Matching requires an open project' });
        ...
        Write explanations in ${req.body?.locale === 'ko' ? 'Korean' : 'English'}.
      ...
      `;
            ...
            if (!Array.isArray(parsed))
                throw new Error("Invalid matching response shape");
            ...
            const matches = parsed.flatMap((item: any) => {
                ...
                return [{
                        ...
                        explanation: String(item.explanation || "").slice(0, 1500),
    ...
    app.post("/api/ai/student-roadmap", async (req: any, res: any) => {
        ...
        try {
            ...
            if (projectError)
                throw projectError;
            if (!profileRow?.data)
                return res.status(404).json({ error: "Complete your student profile before generating a roadmap" });
            ...
            if (!Array.isArray(parsed.milestones) || !parsed.milestones.length)
                throw new Error('AI roadmap has no milestones');
    ...
    app.post("/api/ai/resume-review", async (req: any, res: any) => {
        ...
        try {
            ...
            if (error)
                throw error;
            if (!profileRow?.data)
                return res.status(404).json({ error: "Complete your student profile before requesting a resume review" });
            const profile = profileRow.data as Record<string, any>;
    ...
    app.post("/api/ai/security", async (req: any, res: any) => {
      ...
      `;
            ...
            const text = response.text;
            if (!text)
                throw new Error("Empty security response");
