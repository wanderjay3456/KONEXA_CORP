  ...
  useEffect(() => {
    ...
    void fetch(`/api/gemini/chat-history?contextKey=${encodeURIComponent(contextKey)}`, { signal: request.signal })
      ...
      .then(body => {
        ...
        if (epoch.current === run && !request.signal.aborted) error('AI', locale === 'ko' ? '이전 대화를 불러오지 못했습니다. 새로고침 후 다시 확인해 주세요.' : locale === 'vi' ? 'Không tải được lịch sử. Hãy tải lại trang.' : 'Could not restore the conversation. Please reload and retry.');
      }).finally(() => { clearTimeout(timeout); if (epoch.current === run) setRestoring(false); });
  ...
  const send = async () => {
    ...
    try {
      ...
      if (epoch.current === run) {
        ...
        error('AI', locale === 'ko' ? '답변을 완료하지 못했습니다. 입력은 보존했습니다. 시간 초과였다면 대화를 다시 불러와 저장 여부를 확인해 주세요.' : locale === 'vi' ? 'Chưa hoàn tất trả lời. Nội dung nhập đã được giữ lại. Nếu hết thời gian, tải lại lịch sử để kiểm tra.' : 'The reply did not complete. Your draft is preserved. After a timeout, reload the conversation to check whether it was saved.');
      }
    } finally { clearTimeout(timeout); if (epoch.current === run) { sending.current = false; setBusy(false); } }
  };
  return { messages, input, setInput, busy, restoring, model, send, reload: () => { if (!sending.current) setRevision(value => value + 1); } };
}
