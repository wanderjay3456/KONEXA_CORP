...
export function authErrorMessage(cause: unknown, locale: Locale): string {
    ...
    if (/invalid.*(credentials|password)|invalid login|wrong.password/i.test(message))
        return t.invalidCredentials;
    if (/email.not.confirmed|email.*confirm/i.test(message))
        return t.unconfirmed;
    if (/rate.limit|too many|429/i.test(message))
        return t.rateLimited;
    if (/fetch|network|timeout|timed out/i.test(message))
        return t.networkError;
    if (/expired|invalid.*(token|link)|otp_expired/i.test(message))
        return t.expired;
    if (/admin|suspend|권한|승인된 관리자|이용.*제한/i.test(message))
