-- Remove the demo marketplace records shipped with the first prototype.
delete from public.app_records
where collection_name = 'applications'
  and ((data->>'projectId') like 'proj_seed_%' or data->>'companyId' = 'seed');

delete from public.app_records
where collection_name = 'projects'
  and (record_id like 'proj_seed_%' or data->>'companyId' = 'seed' or lower(data->>'companyName') = 'konexa labs');
...
begin
  ...
  if new.collection_name = 'projects'
    ...
    entitlement := jsonb_build_object(
      ...
      'role', 'student',
      'campaign', 'early_bird_2026_08_05',
      'qualifiedBy', 'resume_and_intro_video',
      'qualifiedAt', (extract(epoch from clock_timestamp()) * 1000)::bigint,
      'badge', 'Early Pioneer',
...
drop trigger if exists konexa_early_bird_trigger on public.app_records;
create trigger konexa_early_bird_trigger
after insert or update of data on public.app_records
for each row
execute function private.konexa_award_early_bird();

-- Backfill students who already completed both requirements before this migration.
insert into public.app_records (collection_name, record_id, owner_id, data, is_public)
select
  'early_bird_entitlements',
  'student:' || (data->>'uid'),
  (data->>'uid')::uuid,
  jsonb_build_object(
    'id', 'student:' || (data->>'uid'),
...
where collection_name = 'student_profiles'
  and coalesce(data->>'uid', '') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
  and nullif(btrim(data->>'resumeUrl'), '') is not null
  and nullif(btrim(data->>'introVideoPath'), '') is not null
