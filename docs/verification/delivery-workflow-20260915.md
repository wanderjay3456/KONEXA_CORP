...
A real-provider follow-up still returned HTTP 400 (surfaced as a safe 503), so the localization path was not declared recovered. The legacy temperature override is removed in line with [Google's Gemini 3 configuration guidance](https://ai.google.dev/gemini-api/docs/gemini-3#temperature). Diagnostics retain only allowlisted configuration field names, never provider payloads. A successful real preview request is required before promoting this compatibility refinement.

Removing the temperature override alone did not recover the real request. A controlled follow-up changed only the provider/client deadlines: the same protected-preview translation probe then returned HTTP 200 with two valid Korean translations in approximately 2 seconds. The installed SDK forwards its timeout as an `X-Server-Timeout` header, not only a local cancellation timer. The production and preview help-provider paths also returned saved, provider-routed responses. Tests now preserve the verified 12-second provider budget and client headroom. Live Korean, English and Vietnamese requests and a 20-string batch subsequently returned HTTP 200. Standalone product/qualification labels are now preserved by shared browser/server logic rather than entrusted to model instructions. Browser fixtures remain a separate check from these live-provider requests.
